...
find "$config_dir" -type f -name "*.yaml" | while read -r yaml_file; do
    echo "Running command sbatch workflow_scripts/train_job.sh \"python\" \"src/train.py $yaml_file\""
    sbatch workflow_scripts/train_job.sh "python" "src/train.py $yaml_file"
done
