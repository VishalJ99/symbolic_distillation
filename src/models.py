...
class HGN(MessagePassing):
    ...
    def loss(
        self, g, augment=True, square=False, reg=True, augmentation=3, **kwargs
    ):
        all_derivatives = self.just_derivative(
            g, augment=augment, augmentation=augmentation
        )
        ndim = self.ndim
        ...
        if reg:
            ...
            regularization = 1e-6 * (
                torch.sum((total_energy) ** 2) + torch.sum((dH_dother) ** 2)
            )
